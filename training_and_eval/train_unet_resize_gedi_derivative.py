...
def train_vgg16(train_dir=None, validation_dir=None):
    ...
    with tf.device('/gpu:0'):
        with tf.variable_scope('cnn') as scope:
            ...
            if config.ordinal_classification is None:
                ...
                cost = tf.nn.l2_loss(tf.squeeze(vgg.output) - train_labels)
            ...
            if config.ordinal_classification is None:
                ...
                train_accuracy = tf.nn.l2_loss(
                    tf.squeeze(vgg.output) - train_labels)
            tf.summary.scalar("training accuracy", train_accuracy)
            tf.summary.image(
                "training prediction %s" % extra_im_name,
                tf.concat(
                    [vgg.deconv_output, vgg.deconv_output, vgg.deconv_output],
                    axis=-1))
            ...
            if validation_data is not False:
                ...
                if config.ordinal_classification is None:
                    ...
                    val_accuracy = tf.nn.l2_loss(tf.squeeze(val_vgg.output) - val_labels)
    ...
    try:
        ...
        while not coord.should_stop():
            ...
            _, loss_value, train_acc, train_gedi_loss, train_class_loss, trgedi, trgfp, trdeconv = sess.run(
                [train_op, cost, train_accuracy, gedi_loss, class_loss, train_gedi_images, train_images, vgg.deconv_output])
